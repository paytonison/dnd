...
int level(const Context& x,const std::string& cls){auto it=x.classLevels.find("srd55:"+cls);return it==x.classLevels.end()?0:it->second;}
...
int table(const Context& x,const std::string& cls,const std::string& column,int fallback){const auto* data=x.rules.find("srd55:"+cls);if(!data)return fallback;const auto* value=at(*data,"/progression/"+column+"/"+std::to_string(level(x,cls)-1));return value&&value->is_number_integer()?value->get<int>():fallback;}
int tableDie(const Context& x,const std::string& cls,const std::string& column,int fallback){const auto* data=x.rules.find("srd55:"+cls);if(!data)return fallback;const auto value=text(*data,"/progression/"+column+"/"+std::to_string(level(x,cls)-1));const auto d=value.find('d');if(d==std::string::npos)return fallback;try{return std::stoi(value.substr(d+1));}catch(...){return fallback;}}
...
bool subclass(const Context& x,const std::string& cls){auto id=text(x.document.choices,"/subclasses/"+cls);if(id.empty() && x.classLevels.size()==1)id=text(x.document.choices,"/subclassId");const auto* item=x.rules.find(id);return level(x,cls)>=3 && item && item->value("kind","")=="subclass" && item->value("classId","")=="srd55:"+cls;}
...
struct Builder {
    ...
    int characterLevelAt(int classLevel)const{const auto events=x.classLevelEvents.find("srd55:"+cls);return events!=x.classLevelEvents.end()&&classLevel>0&&events->second.size()>=static_cast<std::size_t>(classLevel)?events->second.at(static_cast<std::size_t>(classLevel-1)):classLevel;}
    void calc(const std::string& key,const std::string& label,Json value,const std::string& reason,const std::string& source=""){addCalculation(out.evaluation,"feature."+cls+"."+key,label,std::move(value),{reason},{srd55v2::ref(source.empty()?page:source)});}
    ...
    void resource(const std::string& key,const std::string& label,int maximum,const std::string& recharge,const std::string& detail=""){
        const auto id=cls+":"+key;out.evaluation.resources.push_back({id,label,maximum,recharge,{srd55v2::ref(page)}});
    ...
    void grant(const std::string& spell,const std::string& ability,const std::string& reason,int freeUses=0,const std::string& recharge="long-rest"){
        ...
        out.spellGrants.push_back({{"spellId",id},{"classId","srd55:"+cls},{"ability",ability},{"reason",reason},{"freeUses",freeUses},{"recharge",recharge},{"source",{{"publication","System Reference Document 5.2.1"},{"page",page},{"url","https://media.dndbeyond.com/compendium-images/srd/5.2/SRD_CC_v5.2.1.pdf"}}}});
    }
    ...
    void skills(const std::string& key,const std::string& label,int count,bool expertise,int grantClassLevel,const std::set<std::string>& allowed={}){
        ...
        for(const auto& id:multi(key,label,std::move(choices),count)){
            if(expertise)out.expertise.insert(id);
            else {out.skillProficiencies.insert(id);const auto previous=out.skillAcquisitionLevels.find(id);if(previous==out.skillAcquisitionLevels.end())out.skillAcquisitionLevels[id]=grantCharacterLevel;else previous->second=std::min(previous->second,grantCharacterLevel);}
        }
    }
};
...
FeatureResult advancementAbilityGrant(const std::string& classId,int classLevel){
    ...
    if(classId=="srd55:barbarian"){result.abilityBonuses={{"strength",4},{"constitution",4}};result.abilityCaps={{"strength",25},{"constitution",25}};}
    if(classId=="srd55:monk"){result.abilityBonuses={{"dexterity",4},{"wisdom",4}};result.abilityCaps={{"dexterity",25},{"wisdom",25}};}
    return result;
}
...
FeatureResult resolveClassChoices(const Context& sourceX){
    ...
    if(level(x,"warlock")>=2){
        int acquiredAt=2;const auto events=x.classLevelEvents.find("srd55:warlock");if(events!=x.classLevelEvents.end()&&events->second.size()>=2)acquiredAt=events->second[1];
    ...
    FeatureResult out;
    for(const auto& [classId,classLevel]:x.classLevels){const auto grant=advancementAbilityGrant(classId,classLevel);for(const auto& [ability,bonus]:grant.abilityBonuses)out.abilityBonuses[ability]+=bonus;for(const auto& [ability,cap]:grant.abilityCaps)out.abilityCaps[ability]=cap;}
    ...
    if(int l=level(x,"cleric")){
        ...
        if(order=="protector"){out.weaponTraining.insert("martial");out.armorTraining.insert("heavy");}else if(order=="thaumaturge")out.cantripBonuses["srd55:cleric"]++;
    ...
    if(int l=level(x,"druid")){
        ...
        if(order=="warden"){out.weaponTraining.insert("martial");out.armorTraining.insert("medium");}else if(order=="magician")out.cantripBonuses["srd55:druid"]++;
...
FeatureResult evaluateClassFeatures(const Context& x){
    ...
    if(int l=level(x,"barbarian")){
        ...
        if(!x.armored)out.armorFormulas["Barbarian Unarmored Defense"]=10+dexterity+constitution;
        ...
        if(l>=3&&rage)for(const auto& key:{"acrobatics","intimidation","perception","stealth","survival"})out.skillAbilityOverrides["srd55:"+std::string(key)]="strength";
        if(l>=5){out.attackCount=std::max(out.attackCount,2);if(x.armorCategory!="heavy")out.speedBonus+=10;}
    ...
    if(int l=level(x,"bard")){
        ...
        if(l>=2)out.halfProficiency=x.proficiency/2;
    ...
    if(int l=level(x,"cleric")){
        Builder b(x,out,"cleric","37-40",true);const auto order=text(x.document.choices,b.path("divineOrder"));if(order=="thaumaturge"){out.skillBonuses["srd55:arcana"]+=std::max(1,wisdom);out.skillBonuses["srd55:religion"]+=std::max(1,wisdom);b.calc("thaumaturgeKnowledge","Thaumaturge Arcana/Religion check bonus",std::max(1,wisdom),"Wisdom modifier, minimum+1, added to Intelligence Arcana/Religion checks; distinct from proficiency.");}
    ...
    if(int l=level(x,"druid")){
        Builder b(x,out,"druid","42-46",true);b.grant("speak-with-animals","wisdom","Druidic");if(text(x.document.choices,b.path("primalOrder"))=="magician"){out.skillBonuses["srd55:arcana"]+=std::max(1,wisdom);out.skillBonuses["srd55:nature"]+=std::max(1,wisdom);b.calc("magicianKnowledge","Magician Arcana/Nature check bonus",std::max(1,wisdom),"Wisdom modifier, minimum+1, added to Intelligence Arcana/Nature checks; distinct from proficiency.");}
        ...
        if(l>=7){const auto fury=text(x.document.choices,b.path("elementalFury"));if(fury=="primal-strike")b.mode("primalStrike","Primal Strike",{{"extraDamageDice",l>=15?"2d8":"1d8"},{"types",{"cold","fire","lightning","thunder"}}},"Once on your turn when a weapon or Wild Shape Beast attack hits.");else if(fury=="potent-spellcasting")b.mode("potentSpellcasting","Druid Potent Spellcasting",{{"classId","srd55:druid"},{"spellLevel",0},{"damageBonus",wisdom},{"rangeBonusFeet",l>=15?300:0},{"rangeBonusMinimumBaseFeet",10}},"Add Wisdom to Druid cantrip damage. At15, cantrips with base range10+ feet gain300 feet.");}
    ...
    if(int l=level(x,"fighter")){
        Builder b(x,out,"fighter","47-49",true);b.resource("second-wind","Second Wind",table(x,"fighter","secondWind",l>=10?4:l>=4?3:2),"short-rest:1;long-rest:all");b.calc("secondWindHealing","Second Wind healing","1d10 + "+std::to_string(l),"Bonus Action; spend one Second Wind use.");out.attackCount=std::max(out.attackCount,l>=20?4:l>=11?3:l>=5?2:1);
    ...
    if(int l=level(x,"monk")){
        Builder b(x,out,"monk","50-52",true);const int die=tableDie(x,"monk","martialArtsDie",l>=17?12:l>=11?10:l>=5?8:6);const bool unarmored=!x.armored&&!x.shield;if(unarmored)out.armorFormulas["Monk Unarmored Defense"]=10+dexterity+wisdom;
    ...
    if(int l=level(x,"ranger")){
        ...
        if(l>=5)out.attackCount=std::max(out.attackCount,2);if(l>=6){if(x.armorCategory!="heavy")out.speedBonus+=10;b.mode("roving","Roving",{{"speedBonusFeet",x.armorCategory!="heavy"?10:0},{"climbSpeedEqualsWalking",true},{"swimSpeedEqualsWalking",true}},"+10 walking Speed without Heavy armor; gain Climb and Swim Speeds equal to Speed.");}
    ...
    if(int l=level(x,"sorcerer")){
        Builder b(x,out,"sorcerer","65-70",true);b.resource("innate-sorcery","Innate Sorcery",2,"long-rest:all");b.effectSwitch("innate-sorcery","Innate Sorcery active");const bool innate=active(x,"sorcerer:innate-sorcery");if(innate)out.spellSaveBonuses["srd55:sorcerer"]+=1;
        b.mode("innateSorcery","Innate Sorcery",{{"active",innate},{"classId","srd55:sorcerer"},{"saveDCBonus",1},{"spellAttackAdvantage",true},{"durationMinutes",1},{"action","bonus-action"}},"Affects Sorcerer spells only; spend one Innate Sorcery use.");
        ...
        if(subclass(x,"sorcerer")){
            out.hpBonus+=l;if(!x.armored)out.armorFormulas["Draconic Resilience"]=10+dexterity+charisma;b.calc("draconicHp","Draconic Resilience HP bonus",l,"+3 at Sorcerer3, +1 per later Sorcerer level; class levels only.");
    ...
    if(int l=level(x,"wizard")){
        ...
        if(subclass(x,"wizard")){
            ...
            if(l>=10)b.mode("empoweredEvocation","Empowered Evocation",{{"classId","srd55:wizard"},{"school","evocation"},{"damageBonus",intelligence},{"maximumDamageRollsPerSpell",1}},"Add Intelligence to one damage roll of a Wizard Evocation spell.");
    ...
    for(const auto& cls:{"druid","warlock"}) {
        ...
        Builder b(x,out,cls,cls==std::string("druid")?"43":"73-74",true);auto profile=*creature;profile["ownerClass"]="srd55:"+std::string(cls);profile["originalType"]=profile.value("type","");profile["type"]=cls==std::string("druid")?"Fey":text(x.document.resources,"/familiars/warlock/type","Fey");profile["normalAttackAllowed"]=false;
    ...
    for(const auto& [classId,l]:x.classLevels){const auto cls=slug(classId);const auto* data=x.rules.find(classId);if(!data)continue;std::vector<std::string> notes;for(const auto& feature:data->value("features",Json::array()))if(feature.value("level",99)<=l)notes.push_back(feature.value("name","")+" [SRD "+feature.value("source",Json::object()).value("page","")+"]");std::vector<std::string> ids;for(const auto& calc:out.evaluation.calculations)if(calc.id.starts_with("feature."+cls+"."))ids.push_back(calc.id);out.evaluation.sections.push_back({data->value("name",cls)+" features",ids,notes});}
    return out;
}
} // namespace dnd::srd55v2
