...
bool invocation(const CharacterDocument& d,const ResolvedRuleset& rules,const std::string& id){if(profileLevel(d.choices,rules,"warlock")==0)return false;const auto known=strings(d.choices,"/features/warlock/invocations");return std::find(known.begin(),known.end(),"srd55:"+id)!=known.end();}
...
bool canCast(const CharacterDocument& d,const ResolvedRuleset& rules,const Evaluation& e,bool noMaterials){
    ...
    if(!text(d.resources,"/wildShapeForm").empty()&&(profileLevel(d.choices,rules,"druid")<18||!noMaterials))return false;
    return true;
}
...
std::vector<Route> routes(const CharacterDocument& d,const ResolvedRuleset& rules,const Evaluation& e){
    ...
    const auto* book=at(e.moduleData,"/spellbooks/wizard");if(profileLevel(d.choices,rules,"wizard")>0&&book&&book->is_array()&&std::find(book->begin(),book->end(),Json(spellId))!=book->end())add({"wizard-book-ritual","Find Familiar — Wizard spellbook ritual","wizard","intelligence","wizard-book","","ritual","78, 104, 130",70,1000,false,false});
    if(invocation(d,rules,"pact-of-the-chain"))add({"pact-chain","Find Familiar — Pact of the Chain","warlock","charisma","pact-of-the-chain","","magic-action","74, 130",0,1000,false,false});
    if(profileLevel(d.choices,rules,"druid")>=2){add({"wild-companion-shape","Find Familiar — spend Wild Shape","druid","wisdom","wild-companion","druid:wild-shape","magic-action","43",0,0,false,true});add({"wild-companion-slot","Find Familiar — Wild Companion with a slot","druid","wisdom","wild-companion","","magic-action","43",0,0,true,true});}
    const auto* profiles=at(e.moduleData,"/castingProfiles");if(profiles&&profiles->is_array())for(std::size_t index=0;index<profiles->size();++index){const auto& p=(*profiles)[index];if(!p.is_object()||p.value("spellId","")!=spellId||p.value("sourceType","")=="item")continue;
        const auto key=p.value("profileId","");if(key.empty())continue;const auto ability=p.value("ability","");if(ability.empty())continue;const auto reason=p.value("reason",p.value("classId","Spell grant"));const auto owner=classProfile(rules,p.value("classId",""))=="wizard"?"wizard":classProfile(rules,p.value("classId",""))=="warlock"?"warlock":"spell";
...
std::string routeFailure(const CharacterDocument& d,const ResolvedRuleset& rules,const Evaluation& e,const Route& route){
    if(!otherwiseValid(e))return "Resolve the character's rule errors before casting.";
    if(!canCast(d,rules,e,route.materialCp==0))return "The character cannot currently cast this spell (HP, condition, untrained armor, Rage, or Wild Shape restriction).";
...
Json spiritProfile(const CharacterDocument& d,const ResolvedRuleset& rules,const Evaluation& e,const Familiar& f,const Json& creature){
...
void appendSrd55CompanionState(const CharacterDocument& d,const ResolvedRuleset& rules,Evaluation& e){
    ...
    for(const auto& f:current){const auto form=f.record.value("form","");const auto* creature=rules.find(form);if(!creature||creature->value("kind","")!="creature"){error(e.messages,"source-missing","The saved familiar form is unavailable; its selection is retained.","/resources/familiars/"+f.owner+"/form");continue;}
        ...
        profiles.push_back(spiritProfile(d,rules,e,f,*creature));
    }
...
void appendSrd55CompanionActions(const CharacterDocument& d,const ResolvedRuleset& rules,Evaluation& e){
    ...
    Json effectiveProfiles=Json::array();for(const auto& familiar:familiars(d))if(const auto* creature=rules.find(familiar.record.value("form",""));creature&&creature->value("kind","")=="creature")effectiveProfiles.push_back(spiritProfile(d,rules,e,familiar,*creature));
    ...
    const bool chain=invocation(d,rules,"pact-of-the-chain");const auto formChoices=forms(rules,chain);const auto spiritChoices=std::vector<Choice>{{"Celestial","Celestial",true,{}, {srd55v2::ref("130")}},{"Fey","Fey",true,{}, {srd55v2::ref("130")}},{"Fiend","Fiend",true,{}, {srd55v2::ref("130")}}};
    for(const auto& route:routes(d,rules,e)){
        auto reason=routeFailure(d,rules,e,route);std::vector<Field> fields={select("/form","Familiar form",formChoices)};if(!route.druid)fields.push_back(select("/type","Spirit type",spiritChoices));if(route.slot)fields.push_back(select("/slotResource","Spell slot to expend",slotOptions(d,e)));fields.push_back(integer("/minutes","Accepted casting time (minutes)",route.minimumMinutes,1000000,route.minimumMinutes==0?"This feature casts with a Magic action; zero completed minutes is valid.":"Rituals take the normal hour plus10 minutes."));fields.push_back(boolean("/completed","Casting completed"));fields.push_back(boolean("/interrupted","Casting was interrupted"));
