...
Current B/X test coverage: **12 test cases, 9,866 assertions** passed on the development Mac after the implementation and source corrections. The authoritative command is `build/dnd_tests '[bx]'`; the total may grow as regressions are added. GUI workflow and persistence tests are owned by the shared application test suite rather than this ledger.

## Explicit interpretations and DM decisions

These are not presented as additional verified book rules:

1. **Beyond level 14 — first-release gate remains open:** X7 and X8 were visually rechecked during the plan follow-through against the same original Expert snapshot identified above. X7 explicitly gives Fighter and Magic-user a maximum of 36; X8 permits Cleric, Fighter, Magic-user and Thief continuation through 36. Beyond level 14, each additional level requires +100,000 XP/+1 HP for Cleric, +120,000 XP/+2 HP for Fighter, +150,000 XP/+1 HP for Magic-user, and +120,000 XP/+2 HP for Thief. The page also provides attack improvement guidance with a minimum target of 2. Later spell and class-ability progression is presented as suggested DM development pending Companion, not complete printed tables. The executable module still supports the complete X5/X6 tables through human level 14 and now says **Printed-table coverage limit** in its sheet and level help. It rejects higher inputs explicitly while preserving them. This is an accurately labeled limitation, not completion of the normal-maximum requirement. A complete continuation profile, including the unresolved spell, save, turning and thief-ability treatment, must be specified before raising that boundary. No BECMI, retroclone, or invented level-36 table has been substituted.
