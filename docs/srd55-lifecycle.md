...
## Physical Wizard spellbooks

Physical-book tracking is an explicit action in the expanded `2.0.0` module. After beginning owned inventory and accepting the completed character history, **Track physical Wizard spellbooks** (`srd55.spellbooks.initialize`) binds the existing learned spells to the exact original book granted by Wizard Spellcasting. This includes a character using the gold starting option. Recalculation and reopening never initialize books or reconstruct missing contents.

Each registered book belongs to one inventory instance and contains its own spell list and writing receipts. An acquired blank book is registered with `srd55.spellbooks.register`; it receives no automatic spells. Acquisition records the campaign's actual gift or agreed price because the inspected SRD does not supply a separate replacement-book purchase price. `srd55.spellbooks.destination` chooses the carried book receiving later Wizard/Savant level grants and external copies. Backups do not automatically receive new research. Gaining the first Wizard level after owned inventory has begun records the class-granted initial book at advancement commit, exactly once.

`srd55.spellbooks.copy` copies selected spells between different carried registered books, or reconstructs currently prepared Wizard spells after a recorded book loss. Both procedures require **one hour and 10 GP per spell level**. The action records its exact source and destination, spells, accepted time, material payment, and resulting balance together. For example, copying Shield (level 1) and Misty Step (level 2) requires **180 minutes and 3,000 cp**. Invalid time, cost, source, duplicate spell, or destination returns the original document unchanged. See [SRD 78](https://media.dndbeyond.com/compendium-images/srd/5.2/SRD_CC_v5.2.1.pdf#page=78).

The existing inventory **Dispose**, **Move**, and **Recover** actions govern possession and access. Lost, destroyed, and stored books retain their exact contents but supply no readable spells. A recovered original book restores only what was actually written in that instance; spells subsequently added to a replacement remain in the replacement. New disposal or storage of an untracked original book requires beginning physical tracking first. This prevents loss from leaving the book's effects active merely because tracking was not enabled.

Losing a book preserves accepted normal preparations, Spell Mastery, and Signature Spells. The character remains usable, can spend slots and rest, and can reconstruct those still-prepared Wizard spells. Once a retained spell is replaced without being written into another book, it no longer qualifies for reconstruction. Origin-only, innate, and other-class preparations do not become Wizard-book entries merely because the spell also appears on the Wizard list. New normal preparations, Memorize Spell replacements, and Spell Mastery replacements require readable book contents and their existing rest cadence. Book-ritual casting requires the actual spell in a carried book; Arcane Recovery requires an accessible book to study. See [SRD 78–79](https://media.dndbeyond.com/compendium-images/srd/5.2/SRD_CC_v5.2.1.pdf#page=78) and [104](https://media.dndbeyond.com/compendium-images/srd/5.2/SRD_CC_v5.2.1.pdf#page=104).

External-book and scroll copying retain their original costs and scroll checks. While physical tracking is active, their duplicate check concerns the selected destination. A previously learned but lost spell can be found and copied again without duplicating its historical acquisition record. Only successful scroll copying writes to the book, while either resolved check outcome still consumes the actual scroll and materials.

Legacy saves are not silently migrated. If an older untracked inventory already records original-book disposal, its exact former contents cannot always be reconstructed from the old data. Such files retain their preparations and original records, and an unavailable original supplies no rituals or study. Initialization will not grant all historical spells to a blank replacement or guess missing migration history. That migration remains explicitly unsupported. The SRD's 100-page book description is retained as source context; no spell-page-capacity formula is invented because the inspected rules do not define one.

The focused public-command regressions in `tests/test_srd55_spellbooks.cpp` cover independent copying arithmetic, failure rollback, loss and reconstruction, source ownership, preparation retention and replacement, origin-spell separation, external recopying, exact recovery, physical advancement, alternate supported Wizard progression, and save/reopen. Native and PDF acceptance are recorded separately for the built revision; these tests do not establish complete SRD lifecycle coverage.
